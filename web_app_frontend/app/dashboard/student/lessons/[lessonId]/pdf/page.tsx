...
export default function StudentLessonPdfPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-neutral-950">
      <div className="flex items-center justify-between gap-4 border-b border-white/10 bg-neutral-900/95 px-4 py-3 text-white">
        <div>
          <div className="text-xs uppercase tracking-[0.2em] text-white/50">Student PDF Viewer</div>
          <div className="text-sm font-semibold">{lesson?.title ?? 'Loading PDF…'}</div>
        </div>
        <Button as={Link} href={`/dashboard/student/lessons/${lessonId}`} variant="outline" size="sm" className="text-white">
          Back to material
        </Button>
      </div>
